...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".codesage" / "config.yaml",
        cwd / ".codesage" / "config.yaml",
        cwd / ".codesage" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .codesage/config.yaml "
            "in project or ~/.codesage/config.yaml"
        )
    return merged
