...
from codesage.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from codesage.commands.completion import CompletionPopup
from codesage.commands.handlers import register_all_commands
from codesage.config import MCPServerConfig, ProviderConfig
from codesage.hooks import HookContext, HookEngine, load_hooks
from codesage.conversation import ConversationManager, Message
from codesage.mcp import MCPManager
from codesage.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from codesage.permissions import (
...
from codesage.agents.task_manager import TaskManager
from codesage.agents.trace import TraceManager
from codesage.agents.notification import inject_task_notifications
from codesage.commands.handlers.tasks import create_tasks_command
from codesage.skills.executor import SkillExecutor
from codesage.skills.loader import SkillLoader
from codesage.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from codesage.cache import FileCache
from codesage.tools import ToolRegistry, create_default_registry
from codesage.tools.agent_tool import AgentTool
from codesage.tools.ask_user import AskUserEvent, AskUserTool
from codesage.tools.impl.tool_search import ToolSearchTool
from codesage.tools.load_skill import LoadSkill
from codesage.worktree.cleanup import start_stale_cleanup_task
from codesage.worktree.manager import WorktreeManager
from codesage.commands.handlers.worktree import create_worktree_command
from codesage.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".codesage", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".codesage" / "history"
...
class CodeSageApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_CODESAGE_THEME)
        self.theme = "codesage"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".codesage" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".codesage" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".codesage" / "permissions.local.yaml",
            ),
        ...
        from codesage.filehistory import FileHistory
        ...
        from codesage.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from codesage.config import WorktreeConfig
        ...
        from codesage.tools.enter_worktree import EnterWorktreeTool
        from codesage.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from codesage.teams.manager import TeamManager
        from codesage.tools.team_create import TeamCreateTool
        from codesage.tools.team_delete import TeamDeleteTool
        ...
        from codesage.commands.handlers.trace import create_trace_command
        ...
        from codesage.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from codesage.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from codesage.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from codesage.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from codesage.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from codesage.permission_dialog import InlinePermissionWidget
