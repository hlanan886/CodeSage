...
def _session_path(codesage_dir: Path) -> Path:
    return codesage_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(codesage_dir)
...
def load_worktree_session(codesage_dir: Path) -> WorktreeSession | None:
    path = _session_path(codesage_dir)
    if not path.exists():
        return None
    try:
