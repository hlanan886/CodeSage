...
from codesage.config import ConfigError, load_config
from codesage.hooks import HookConfigError, HookEngine, load_hooks
from codesage.permissions import PermissionMode


def main() -> None:
    # 先确保 .codesage/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".codesage").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".codesage/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="codesage", description="CodeSage AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from codesage.app import CodeSageApp
    from codesage.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    ...
    from codesage.client import create_client, resolve_context_window
    from codesage.conversation import ConversationManager
    from codesage.memory.instructions import load_instructions
    ...
    from codesage.tools import create_default_registry
