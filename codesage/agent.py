...
from codesage.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from codesage.conversation import ThinkingBlock as ConvThinkingBlock
from codesage.memory.auto_memory import MemoryManager
from codesage.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from codesage.hooks import HookContext, HookEngine, ToolRejectedError
from codesage.hooks.engine import HookNotification
from codesage.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from codesage.tools import ToolRegistry
from codesage.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".codesage" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".codesage" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from codesage.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from codesage.context.manager import (
